...
const GenericNode = React.memo(
	({ data: Data, type, triggerPosition, labelComponent }: GenericNodeProps) => {
		...
		const signalsLabel = Data.supported_signals || [];
					...
					<div onClick={() => setIsOpen(true)} className="flex items-center gap-0">
						{type === "source" && (
							<>
								...
								<div className="bg-gray-200 p-2 rounded-md flex flex-col items-center justify-center h-[4rem] w-[8rem]">
									<Handle
										type="source"
										position={Position.Right}
										className="bg-green-600 w-0 h-0 rounded-full"
									/>
									<div className="text-[9px] font-medium text-center break-words max-w-full">
										{Data.name}
									</div>
						...
						{(type === "processor" || type === "destination") && (
							<>
								<div className="bg-gray-200 p-2 rounded-md flex flex-col items-center justify-center h-[4rem] w-[8rem]">
									<Handle
										type="target"
